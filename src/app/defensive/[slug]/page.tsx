...
const fetchDefensiveSecurityData = async (slug: string) => {
  const url = `${process.env.NEXT_PUBLIC_BASE_API_URL}/api/defensive-securities/?filters[slug][$eq]=${slug}&populate=deep`;
  try {
  ...
  return (
    <div>
      ...
      <ContentWithOneSection
        data={data?.attributes?.defensive_security_page[2]}
      />
